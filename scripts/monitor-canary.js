...
async function monitorCanary() {
  ...
  while (Date.now() < endTime) {
    ...
    try {
      ...
      log(
        `[${elapsed}s] Error Rate: ${errorRate.toFixed(2)}% | P95 Latency: ${p95Latency.toFixed(0)}ms | Req/s: ${requestRate.toFixed(2)}`,
        'info'
      );
      ...
      if (errorRate > ERROR_THRESHOLD) {
        ...
        failures++;
        log(
          `P95 latency ${p95Latency.toFixed(0)}ms exceeds threshold ${LATENCY_THRESHOLD}ms`,
          'warning'
        );
