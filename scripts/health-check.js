...
const SERVICES = (process.env.SERVICES || 'user-service,product-service,order-service,payment-service').split(',');
...
async function checkServiceHealth(serviceName) {
  ...
  try {
    ...
    if (response.data && response.data.status === 'healthy') {
      ...
      return { healthy: false, service: serviceName, error: 'Service not healthy', data: response.data };
    }
  } catch (error) {
...
async function checkDatabaseConnectivity() {
  ...
  if (dbIssues.length > 0) {
    log(`⚠️  Database connectivity issues detected in: ${dbIssues.map(i => i.service).join(', ')}`, 'yellow');
    return false;
  }
...
async function performHealthCheck() {
  ...
  log(`Metrics available: ${Object.values(results.metrics).filter(m => m.available).length}`, 'blue');
  log('='.repeat(60), 'blue');
